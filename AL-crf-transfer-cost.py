...
if args.ber_task:
    ...
    selected_modules = ["sentence_cnn","marginal_prob_cnn","labeled_pool","r_lab_pool","policy_net", "reward_net","entropy_cnn", "entropy_embedding"]
    num_classes = 9
else:
...
for tau in range(0, args.timesteps):
    ...
    t = 0
    while t < BUDGET:
        logger.info(' * Repetition: {} Budget so far: {}'.format(str(tau+1), str(t)))
        '''
        queryscores=[]
        states=[]
        ...
        logger.info(" random pool : {}".format(str(len(random_pool))))
        logger.info(" state[0] : {}".format(str(len(state[0]))))
        action, cost=agent.predict(k, state, payments)
        #action, cost=agent.predict(args.k_learning, len(payments), state)
        cost = payments[cost]
        t+=cost
        # logger.info(action)
        ...
        para = 1     
        pay = cost   
        seq = utilities.flip_seq(train_pool[theindex], utilities.para_curve(pay, 'linear',p=para), num_classes)
        #train_la.append(train_pool[theindex])
        train_la.append(seq)
    ...
    f1score=model.test(test_sents, label2str)
    f1list.append(f1score)
    logger.info(" F1 score : {}".format( str(f1score)))
