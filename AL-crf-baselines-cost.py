...
for tau in range(0, 20):
    ...
    t = 0
    while t < BUDGET:
        logger.info('Episode: '+str(tau+1)+' Budget so far: '+str(t))
        ...
        # random sample a cost 
        pay = np.random.choice(payments)
        para = 1
        seq = utilities.flip_seq(sampledata[0], utilities.para_curve(pay, 'linear',p=para), num_classes)
        seq = [seq]
        #train_la = train_la + sampledata
        train_la = train_la + seq
