...
mkdir -p $OUTPUT
echo "TRAIN CEAL POLICY ${POLICY_NAME} with policy path ${POLICY_PATH} on dataset ${DATASET_NAME} experiment name ${EXP_NAME} "
cd $SRC_PATH && python3 AL-crf-simulation-cost.py --root_dir $ROOT_DIR --dataset_name $DATASET_NAME  \
    ...
    --output $OUTPUT --label_data_size 100 --annotation_budget 3 \
